                ...
                <div class="row bg-white py-1 m-0 rounded">
                        ...
                        @if(Auth::id() == $group->user_id || checkGroupFollowing(Auth::id(), $group->id) > 0) 
                            ...
                            <div class="media">
                                ...
                                <div class="media-body post-feed-section pt-0">
                                    ...
                                    <button 
                                        class="btn-post btn-block text-primary font-weight-bold border-0 input-type" data-user_social_group_id="{{Crypt::encrypt($group->id)}}">
                    ...
                    <div class="col-sm-8 text-right m-auto">
                        @if(Auth::id() == $group->user_id) 
                                        ...
                                        <a data-toggle="modal" data-target="#modalPendingPosts" class="font-weight-bold text-warning px-1">Pending post request ({{ count(getGroupPendingPosts($group->id)) }})</a>
...
<div class="modal fade" id="modalPendingPosts" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered" role="document" style="max-width:98%;">
    <div class="modal-content">
      ...
      <div class="modal-body pt-0">
            @foreach(getGroupPendingPosts($group->id) as $pending)
                <div class="row border-bottom py-1 mt-0">
                    <div class="col-sm-6">
                        <p>{{ $pending->caption }}</p>
                </div> 
                <div class="col-sm-6 text-right">
                    <a 
                    href="{{ route('social.group.request.action.post', ['action' => 'accept', 'post_id' => base64_encode($pending->id)]) }}" 
                    class="text-success font-weight-bold">Accept</a>
                    <a href="{{ route('social.group.request.action.post', ['action' => 'reject', 'post_id' => base64_encode($pending->id) ]) }}" class="text-danger font-weight-bold pl-1">Reject</a>
                </div>
                </div>
            @endforeach  
      </div>
