...
use App\Models\HomeopathBadgesSetting;
    ...
    function badgesInformation($id)
    {
        ...
        $posts=$user->socialPosts()->where('post_type','News')->get()->count();
        $resources=$user->socialPosts()->where('post_type','Resources')->get()->count();
    ...
    function calculateServicePriceTax($price)
    {

        $service_price = round(($price/1.13), 2);
        $tax = round(($price - $service_price), 2);
    ...
    function getcompletedTime($slot = "")
    {
        if($slot)
        {
            $slot = json_decode($slot);
    ...
    function getGroupPendingPosts($id)
    {
        return SocialPost::where('user_social_group_id', $id)
       ...
       try
       {
          ...
          $crawler = Goutte::request('GET', $url);
            ...
            if($img == '')
            {
                    ...
                    {
                        $img = substr($url, strpos($url, "v=") + 2, 11);
                    }
                    elseif(strpos( $url,"embed/") !== false)
                    {
                        $img =  substr($url, strpos($url, "embed/") + 6, 11);
                    }

                    $img = 'https://img.youtube.com/vi/'.$img.'/mqdefault.jpg';
    ...
    function checkPlanType($plan)
    {

        switch ($plan)
        {
            case env('HOMEOPATH_MONTHLY_PLAN'):
                return $data = [
                    'plan_name'  => 'homeopath_monthly',
                    'plan'  => 'monthly',
                    'price' => 39.99,
                    'subscription' => 'Practitioner (Remport)'
                ];
                break;

            case env('HOMEOPATH_YEARLY_PLAN'):
                return $data = [
                    'plan_name'  => 'homeopath_yearly',
                    'plan'  => 'yearly',
                    'price' => 359.99,
                    'subscription' => 'Practitioner (Remport)'
                ];
            ...
            case env('ADVOCATE_MONTHLY_PLAN'):
                return $data = [
                    'plan_name'  => 'advocate_monthly',
                    'plan'  => 'monthly',
                    'price' => 4.99,
                    'subscription' => 'Advocate (Materia Faction)'
                ];
                break;


            case env('ADVOCATE_YEARLY_PLAN'):
                return $data = [
                    'plan_name'  => 'advocate_yearly',
                    'plan'  => 'yearly',
                    'price' => 39.99,
                    'subscription' => 'Advocate (Materia Faction)'
                ];
